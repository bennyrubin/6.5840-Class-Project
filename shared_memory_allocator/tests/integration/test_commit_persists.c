...
static int run_writer_then_crash(const char* shm_name) {
  ...
  len = cm_mapping_page_size() * 3u;
  th_fill_pattern(base, len, 0x22u);
  if (cm_commit() != CM_OK) {
...
int main(int argc, char** argv) {
  const char* shm_name = "/cm_integration_commit_persists";
  int exit_code;
  int term_sig;
  unsigned char* base;
  size_t len;
  ...
  base = (unsigned char*)cm_mapping_base();
  len = cm_mapping_page_size() * 3u;
  if (th_expect_pattern(base, len, 0x22u) != 0) {
    fprintf(stderr, "committed pattern did not persist across crash\n");
    cm_close();
    return 1;
  }
